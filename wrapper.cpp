...
int t_list[] = {40961, 65537, 114689, 147457, 188417};

void generate_new_keys_(int security_level, int poly_modulus) {
	// PARAMETERS
	if(security_level == 128)
	{
		q_array = coeff_modulus_128(poly_modulus);
	}
	else
	{
		q_array = coeff_modulus_192(poly_modulus);
	}
	...
	// --t
	parms[0]->set_plain_modulus(t_list[0]);
	parms[1]->set_plain_modulus(t_list[1]);
	parms[2]->set_plain_modulus(t_list[2]);
	parms[3]->set_plain_modulus(t_list[3]);
	parms[4]->set_plain_modulus(t_list[4]);

	std::string file_name = "";
	for (int i=0; i<5; i++) {
		// --n
		parms[i]->set_poly_modulus("1x^" + to_string(poly_modulus) +" + 1");
		// --q
...
void initialize_(int security_level, int poly_modulus) {
	// PARAMETERS
...
extern "C"
{
	...
	void generate_new_keys(int security_level, int poly_modulus) {
		generate_new_keys_(security_level, poly_modulus);
	}
	void initialize(int security_level, int poly_modulus) {
		initialize_(security_level, poly_modulus);
	}
